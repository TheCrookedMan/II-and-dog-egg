    ...
    $.get('/template/basket/order.t', { "uid": 9 ,'type':1,'pids':pids}).success(function(data) {
		...
		　　});
			...
			$("#getCoupon").on('click',function(){
				$.post('/user/validCouponList.post', { "uid": 9 ,'allproductamount':30}).success(function(data) {
					var record=data.data;
					var couponList=record.couponList;
					if(couponList.length>0){
						$.get('/template/basket/coupon.t', { "uid": 9 ,'allproductamount':30}).success(function(data) {
							$("#orderMain").hide();
			...
			$("#getAddress").on('click',function(){
				$.get('/template/profile/address.t', { "uid": 9}).success(function(data) {
					data = data.replace(/(^\s+)|(\s+$)/g, "");
        			if ("" !== data) {
        				$("#orderMain").hide();
